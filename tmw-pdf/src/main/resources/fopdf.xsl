...
<!--	<xsl:param name="body.font.family" select="'serif'" /> -->
	...
	<xsl:param name="body.font.title" select="'12'" />
    ...
    <xsl:template name="footer.content">
		...
		<xsl:variable name="Version">
			<xsl:if test="//releaseinfo">
				<xsl:text>The Maven Way (</xsl:text><xsl:value-of select="//releaseinfo" /><xsl:text>)</xsl:text>
			</xsl:if>
		</xsl:variable>
        <xsl:choose>
...
<xsl:attribute-set name="formal.object.properties">
   <xsl:attribute name="keep-together.within-column">auto</xsl:attribute>
    ...
    <xsl:attribute-set name="formal.title.properties" use-attribute-sets="normal.para.spacing">
        <xsl:attribute name="font-weight">bold</xsl:attribute>
        <xsl:attribute name="font-size">
            <xsl:value-of select="$body.font.title"/>
            <xsl:text>pt</xsl:text>
        </xsl:attribute>
        <xsl:attribute name="hyphenate">false</xsl:attribute>
...
<xsl:template match="d:caution|d:important|d:note|d:warning|d:tip" mode="admon.graphic.width">
